"""Short-only dual EMA proximity follower on completed 5m candles."""
...
def evaluate_symbol(bars, *, asset, symbol, cutoff):
    ...
    pct26 = (ema26 - close) / ema26 * 100; pct99 = (ema99 - close) / ema99 * 100
    if ema26 < ema99 and close < ema99 and close < ema26 and pct26 < config.DUAL_ZONE_SHORT_A_ENTRY_DISTANCE_PCT:
        channel, stop, target = "A", ema26 * (1 + config.DUAL_ZONE_SHORT_A_STOP_DISTANCE_PCT / 100), ema7 * (1 - config.DUAL_ZONE_SHORT_A_TARGET_DISTANCE_PCT / 100)
    elif ema26 < ema99 and close < ema99 and pct99 < config.DUAL_ZONE_SHORT_B_ENTRY_DISTANCE_PCT:
        channel, stop, target = "B", ema99 * (1 + config.DUAL_ZONE_SHORT_B_STOP_DISTANCE_PCT / 100), ema7 * (1 - config.DUAL_ZONE_SHORT_B_TARGET_DISTANCE_PCT / 100)
    else: return None
    observed = row["timestamp"]
    if observed.tzinfo is None: observed = observed.replace(tzinfo=timezone.utc)
    return {"schema_version": 1, "strategy_id": STRATEGY_ID, "asset": asset.upper(), "direction": "short", "setup_class": "dual_zone_short_follower", "phase": f"channel_{channel.lower()}", "observed_at": observed.isoformat(), "valid_until": (observed + timedelta(minutes=5)).isoformat(), "horizon_minutes": 5, "confidence": 0.5, "confidence_status": "uncalibrated", "entry_condition": {"type": "limit_at_ema_context", "price": close}, "entry_price": close, "invalidation_price": stop, "targets": [target], "plugin_version": PLUGIN_VERSION, "feature_snapshot": {"source_symbol": symbol, "execution_timeframe": "5m", "ema7": ema7, "ema26": ema26, "ema99": ema99, "channel": channel, "pct_below_ema26": pct26, "pct_below_ema99": pct99}}
